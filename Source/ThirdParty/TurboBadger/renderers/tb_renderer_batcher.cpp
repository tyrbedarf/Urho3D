...
#define VER_COL_OPACITY(a) (0x00ffffff + (((uint32)a) << 24))
	...
	void TBRendererBatcher::DrawRectFill(const TBRect &dst_rect, const TBColor &color)
	{
		...
		AddQuadInternal(
			...
			TBRect(),
			VER_COL(color.r, color.g, color.b, a),
			nullptr,
			nullptr);
	}

	void TBRendererBatcher::DrawLine(const TBPoint& a, const TBPoint& b, const TBColor& c, int thickness)
	{
		auto dir = b - a;
		auto minX = (int) std::fminl(a.x, b.x);
		auto minY = (int) std::fminl(a.y, b.y);
		auto maxX = (int) std::fmaxl(a.x, b.x);
		auto maxY = (int) std::fmaxl(a.y, b.y);
		TBRect constraint(minX, minY, maxX - minX, maxY - minY);
	...
	void TBRendererBatcher::AddQuadInternal(
...
#if defined(URHO3D_D3D11) || defined(URHO3D_OPENGL)
